...
function brandFacebookPageUrl(b){return typeof b==='string'?'':String(b?.facebookPageUrl||b?.fbPageUrl||b?.facebookUrl||'').trim();}
...
function taskBrandBadgeHtml(task){const b=brandByName(task?.brand||'');const label=brandName(b)||String(task?.brand||'-');const logo=brandLogoUrl(b);if(logo){const src=directImageUrl(logo);return `<div class="task-brand-inline"><span class="task-brand-logo"><img src="${esc(src)}" alt="${esc(label)}" onerror="this.parentElement.classList.add('fallback');this.parentElement.innerHTML='${esc(initials(label))}'"></span><span class="task-brand-text">${esc(label)}</span></div>`;}return `<div class="task-brand-inline"><span class="task-brand-logo fallback">${esc(initials(label))}</span><span class="task-brand-text">${esc(label)}</span></div>`;}
function taskBrandDetailHero(task){const b=brandByName(task?.brand||'');const label=brandName(b)||String(task?.brand||'-');const type=task?.type||'Other';const logo=brandLogoUrl(b);const media=logo?`<span class="task-brand-hero-logo"><img src="${esc(directImageUrl(logo))}" alt="${esc(label)}" onerror="this.parentElement.classList.add('fallback');this.parentElement.innerHTML='${esc(initials(label))}'"></span>`:`<span class="task-brand-hero-logo fallback">${esc(initials(label))}</span>`;return `<div class="task-brand-hero">${media}<div><div class="task-brand-hero-name">${esc(label)}</div><div class="task-brand-hero-sub">${esc(type)}</div></div></div>`;}
...
function financeObj(o={}){const f={id:o.id||Date.now(),date:o.date||today(),product:o.product||'',brand:o.brand||'',owner:o.owner||'',packageName:o.packageName||'',contractMonths:Number(o.contractMonths)||1,packageAmount:Number(o.packageAmount)||0,packageVat:Number(o.packageVat??7)||0,packageWht:Number(o.packageWht??0)||0,extraVat:Number(o.extraVat??7)||0,extraWht:Number(o.extraWht??0)||0,installment:o.installment||'',expenses:normalizeExpenseList(o.expenses,o),paidBreakdown:normalizePaidBreakdown(o.paidBreakdown,o),paymentStatus:o.paymentStatus||o.packageStatus||'pending',paidAmount:Number(o.paidAmount)||0,note:o.note||''};return normalizeFinance(f);}
function normalizeFinance(f={}){const out={...f};out.id=out.id||Date.now();out.date=out.date||today();out.brand=out.brand||'';out.product=out.product||'';out.owner=out.owner||'';out.packageName=out.packageName||'';out.contractMonths=Math.min(12,Math.max(1,Number(out.contractMonths)||1));out.packageAmount=Number(out.packageAmount)||0;out.packageVat=Number(out.packageVat);if(!Number.isFinite(out.packageVat))out.packageVat=7;out.packageWht=Number(out.packageWht);if(!Number.isFinite(out.packageWht))out.packageWht=0;out.extraVat=Number(out.extraVat);if(!Number.isFinite(out.extraVat))out.extraVat=7;out.extraWht=Number(out.extraWht);if(!Number.isFinite(out.extraWht))out.extraWht=0;out.expenses=normalizeExpenseList(out.expenses,out);out.extraSubtotal=round2(expenseTotal(out.expenses));out.extraTaxableSubtotal=round2(expenseTaxableTotal(out.expenses));out.extraVatAmount=round2(out.extraTaxableSubtotal*out.extraVat/100);out.extraWhtAmount=round2(out.extraSubtotal*out.extraWht/100);out.extraTotal=round2(out.extraSubtotal+out.extraVatAmount-out.extraWhtAmount);out.packageWhtAmount=round2(out.packageAmount*out.packageWht/100);out.packageTotal=round2(out.packageAmount+(out.packageAmount*out.packageVat/100)-out.packageWhtAmount);out.paidBreakdown=normalizePaidBreakdown(out.paidBreakdown,out);const breakdownPaid=paidBreakdownTotal(out.paidBreakdown);out.paidAmount=breakdownPaid>0?breakdownPaid:(Number(out.paidAmount)||0);out.grandTotal=round2(out.packageTotal+out.extraTotal);out.balance=round2(Math.max(0,out.grandTotal-out.paidAmount));out.paymentStatus=autoFinanceStatus(out.paymentStatus||out.packageStatus||out.adsStatus,out.paidAmount,out.grandTotal,out.balance);out.note=out.note||'';return out;}
...
function bindFinanceAutoCalc(){
  ...
  if(!modal)return;
  const ids=['fin-package-amount','fin-package-vat','fin-package-wht','fin-paid-amount','fin-payment-status','fin-extra-vat','fin-extra-wht','fin-paid-package',...FIN_COST_TYPES.flatMap(t=>[`fin-cost-${t.key}-check`,`fin-cost-${t.key}`,`fin-paid-${t.key}`])];
  ids.forEach(id=>{
...
openBrandModal = function(name=null){
  ...
  try{
    ...
    const fbPage=document.getElementById('brand-facebook-page-url'); if(fbPage)fbPage.value=b?brandFacebookPageUrl(b):'';
...
saveBrand = async function(){
  ...
  try{
    ...
    let facebookPageUrl=String(document.getElementById('brand-facebook-page-url')?.value||'').trim();if(facebookPageUrl&&!/^https?:\/\//i.test(facebookPageUrl))facebookPageUrl='https://'+facebookPageUrl;
    ...
    const nextBrand=brandObj(name,packageName,contractMonths,tempBrandInfoLinks,logoUrl,tempBrandImages,{customer,product,packageAmount,contentTotal,imageCount,videoCount,channels,facebookPageUrl});
